...
console.log('@@@@@@@@@@@' + mlink);
   ...
   download(mlink,function(err,data){
       ...
       doc.each(function(k){
           //抓取一本书
           //console.log($(this));
           var link = $(this).find(".p-name").find("a").attr('href');
           //console.log(link);
           if(link){
               //解析数据信息，并且入库
               extract(link,mcatlog,function(err, book) {
                   ep.emit('extract-page-list',book);
               });
           }
           else {
               console.log('link nill');
               ep.emit('extract-page-list', null);
           }
           //不在此处触发了。。。
 ...
 * @param catlog
 */
function extract(link,catlog,cb){
...
console.log('------------' + book.url);
                ...
                book.cover_pic  = 'http://' + base.find("#preview #spec-n1 img").attr("src");//
